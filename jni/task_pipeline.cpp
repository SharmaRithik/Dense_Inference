...
double benchmarkLayer(const std::string& layer_name, std::function<void()> layer_func, const std::vector<int>& cores, bool print_layer_timings) {
...
void processInput(const std::string& inputFile, const std::vector<std::vector<int>>& core_allocations, bool print_layer_timings, bool print_linear_layer) {
    // Network parameters
    ...
    const std::vector<std::string> layers = {"Conv1", "MaxPool1", "Conv2", "MaxPool2", "Conv3", "Conv4", "Conv5", "MaxPool3", "LinearLayer"};
    
    for (size_t i = 0; i < layers.size(); ++i) {
        const std::string& layer_name = layers[i];
        const std::vector<int>& cores = core_allocations[i];

        total_time += benchmarkLayer(layer_name, [&]() {
            if (layer_name == "Conv1") {
            ...
            } else if (layer_name == "LinearLayer") {
        ...
        }, cores, print_layer_timings);
    }

    // Output the total time for this core configuration
    std::cout << "Total time for core allocations ";
    for (const auto& core_group : core_allocations) {
        for (int core : core_group) {
            std::cout << core << " ";
        }
    }
...
int main(int argc, char** argv) {
    ...
    std::vector<std::vector<int>> core_allocations = {
        {1}, {2}, {2}, {2}, {1}, // Add more core allocations for the remaining layers
    };
    ...
    for (const auto& entry : fs::directory_iterator(input_dir)) {
        ...
        processInput(entry.path().string(), core_allocations, print_layer_timings, print_linear_layer);
    }
